...
def init_logger(config):
    ...
    init(autoreset=True)
    LOGROOT = './log/'
    LOGROOT += config['model'] + '/{}/bs{}-Oversmoothing{}-Scale{}-Tau{}-{}-lr{}-l2{}/'.format(
        config['dataset'], config['train_batch_size'], config['Oversmoothing'], config['Scale'], config['Tau'],
        get_local_time(), config['learning_rate'], config['weight_decay'])
