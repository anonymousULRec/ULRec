...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask):
        ...
        x = (1 + scale) * x - scale * torch.bmm(sim, x)
        hidden_states = x
        # # print("hidden_states",hidden_states.size()) #torch.Size([256, 5, 64])
        # # exit()
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True):
        ...
        """
        all_encoder_layers = []
        erank = defaultdict(list)
        for layer_module in self.layer:
            hidden_states = layer_module(hidden_states, attention_mask)
            # erank = self.calculate_erank(hidden_states)
            # print("erank",erank)
            # exit()
            if output_all_encoded_layers:
                all_encoder_layers.append(hidden_states)
        # a = {erank}
        # erank = pd.DataFrame(a)
        # # file_name_data = '/home/zhangyabin/Oversmoothing-SR/RecBole-GNN-main/log/LightGCN/erank1contranorm.txt'
        # file_name_data = '/home/zhangyabin/Oversmoothing-SR/RecBole-master/log/SASRec/erank10contranorm.txt'
        # erank.to_csv(file_name_data, sep=' ', index=False)
